...
# ============================================================
# EVE S-01
# Original sports coupe prototype
# ============================================================
...
def collection(name):
    # Reuse collection if it already exists.
    c = bpy.data.collections.get(name)

    if c is None:
        c = bpy.data.collections.new(name)

    # Make sure collection is linked to the scene.
...
def move_to(obj, col):
    # Always resolve the collection from Blender's database.
    # This prevents stale StructRNA references.
...
def cyl(
    ...
    # Do NOT pass rotation into the operator.
    # Blender 5.x can throw a sequence-dimension error there.
...
def create_vehicle():
    ...
    for x, label in [
        ...
        mirror.rotation_euler[2] = math.radians(
            8 if x < 0 else -8
        )
...
def add_vehicle_metadata():
    ...
    root["configurable_components"] = (
        "engine,wheels,brakes,exhaust,"
        "aero,body_panels,interior"
    )
